...
**Base paths.** JSON lives under `/api`. `/healthz` and `/readyz` sit at the root because
whatever probes them will not prepend a prefix. `/g/:token`, the magic link, is also at the
root — but it is a page, the SPA's, not an API route.
...
- Client: the page at `/g/:token` posts the link's token to `POST /api/client/session`, which
  returns a short-lived signed token **in the response body**. The SPA holds
...
| `PATCH`  | `/api/galleries/:galleryId` | `session` | Rename, change `status`, set `coverAssetId` — the photo the gallery opens with. The cover must be a `ready` asset of this gallery (`404 asset_not_found` otherwise); `null` clears it. |
...
| Method   | Path                                           | Auth             | Notes                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| -------- | ---------------------------------------------- | ---------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `POST`   | `/api/client/session`                          | `public`         | The magic link's exchange. `{ token }` in the body — the page at `/g/:token` posts it there, so the long-lived token never travels in a request path. Hashes it, loads the grant, checks `revoked_at` and `expires_at`, stamps `last_seen_at`, and returns `{ token, expiresAt, rights }` — a signed token good for `CLIENT_TOKEN_TTL_SECONDS`. Rate limited per token. `404 grant_not_found` for an unknown token, because a `410` would confirm it once existed; `410 grant_revoked` / `grant_expired` otherwise. |
...
| `GET`    | `/api/client/gallery`                          | `grant:view`     | Gallery metadata plus a keyset page of **`ready`** assets, each with `width`, `height`, `blurhash`, and **`thumbUrl` and `gridUrl` already presigned**. `gallery.coverUrl` is the presigned `preview` of the cover, falling back to the first photo, and is sent only on the first page. `?cursor=` and `?limit=` as on the photographer side. `410` if the gallery has been deleted.                                                                                                                               |
